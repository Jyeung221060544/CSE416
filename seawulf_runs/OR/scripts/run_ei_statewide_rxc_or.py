...
TOTAL_COL = "VAP"
BASE_GROUP_COLS = ["LATINO_VAP", "NH_WHITE_ALONE_VAP", "OTHER_VAP"]
BASE_GROUP_NAMES = ["Latino", "White_NH", "Other"]
...
def extract_posterior_array(
    ...
    if arr.ndim == 4:
        ...
        # expected:
        # (chain, draw, precinct, group, candidate)
        # or
        # (chain, draw, precinct, candidate, group)
...
def build_inputs(gdf: gpd.GeoDataFrame):
    ...
    # PyEI layout: groups x precincts, candidates x precincts
...
def main():
    ...
    (OUTDIR / "ei_OR_rxc_full.json").write_text(json.dumps(full_payload, indent=2))
    (OUTDIR / "ei_OR_rxc_summary.json").write_text(json.dumps(group_results, indent=2))

    print(f"Wrote {(OUTDIR / 'ei_OR_rxc_full.json')}")
    print(f"Wrote {(OUTDIR / 'ei_OR_rxc_summary.json')}")
    print("Done.")
