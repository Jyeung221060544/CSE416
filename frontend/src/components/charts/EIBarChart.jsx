...
function makeBarTooltip(nameMap) {
    return function EIBarTooltip({ id, value, data }) {
        ...
        <div className={`w-full rounded-xl border border-brand-muted/25 shadow-sm bg-white flex flex-col ${className ?? 'h-[320px]'}`}>
            ...
            <div className="flex-1 min-h-0">
                <ResponsiveBar
                    ...
                    margin={{ top:16, right:40, bottom:64, left:72 }} padding={groupPadding} innerPadding={4}
                    ...
                    tooltip={barTooltip} isInteractive
                />
            </div>
        </div>
    )
