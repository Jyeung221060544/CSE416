...
/**
 * Mini horizontal Dem/Rep split bar.
 * The blue segment fills `demPct`% from the left; the rest is red.
 *
 * @param {{ demPct: number }} props
 */
...
/**
 * Bin-level summary table for Gingles racial-polarization analysis.
 * Precincts are binned by minority VAP %; each row shows avg Dem/Rep vote share,
 * a split bar, and an outcome badge. A polarization footer appears when trend direction
 * is computable.
 *
 * @param {{ summaryRows: Array, raceFilter: string|null }} props
 */
    ...
    return (
        <SurfacePanel className="h-[404px] border-brand-muted/25 flex flex-col overflow-hidden">
            ...
            <div className="shrink-0 grid grid-cols-[1fr_40px_60px_60px_88px] items-center px-4 py-2.5 bg-brand-darkest text-brand-surface text-xs font-semibold">
                <span className="text-center">{raceName} VAP Range</span>
                <span className="text-center">n</span>
                <span className={`text-center ${DEM_HEADER_TEXT}`}>Avg Dem</span>
                <span className={`text-center ${REP_HEADER_TEXT}`}>Avg Rep</span>
                ...
                {rows.map((row, i) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className="text-right tabular-nums text-brand-deep text-sm font-medium">
                                {row.precinctCount}
                            </span>
