...
def main():
    ...
    AL = build_precinct_geojson_with_vap(
        vap_csv_path=str(ROOT / "BASE_FILES" / "AL-VAP-population.csv"),
        blocks_shp_path=str(ROOT / "BASE_FILES" / "AL-shapefile" / "tl_2025_01_tabblock20.shp"),
        precincts_geojson_path=str(ROOT / "AL_data" / "AL-precincts-with-results-enacted.geojson"),
        output_geojson_path=str(ROOT / "AL_data" / "AL_precincts_full.geojson"),
        state_code="AL",
        verbose=True,
    )

    OR = build_precinct_geojson_with_vap(
        vap_csv_path=str(ROOT / "BASE_FILES" / "OR-VAP-population.csv"),
        blocks_shp_path=str(ROOT / "BASE_FILES" / "OR-shapefile" / "tl_2025_41_tabblock20.shp"),
        precincts_geojson_path=str(ROOT / "OR_data" / "OR-precincts-with-results-enacted.geojson"),
        output_geojson_path=str(ROOT / "OR_data" / "OR_precincts_full.geojson"),
        state_code="OR",
        verbose=True,
    )
    ...
    AL2 = add_region_type_from_ruca(
        AL,
        tracts_path=str(ROOT / "BASE_FILES" / "AL_tract" / "tl_2025_01_tract.shp"),
        ruca_csv_path=str(ROOT / "BASE_FILES" / "region-type.csv"),
    )
    OR2 = add_region_type_from_ruca(
        OR,
        tracts_path=str(ROOT / "BASE_FILES" / "OR_tract" / "tl_2025_41_tract.shp"),
        ruca_csv_path=str(ROOT / "BASE_FILES" / "region-type.csv"),
    )
    ...
    AL3 = add_income_from_acs_s1901(
        AL2,
        tracts_path=str(ROOT / "BASE_FILES" / "AL_tract" / "tl_2025_01_tract.shp"),
        income_csv_path=str(ROOT / "BASE_FILES" / "AL-income.csv"),
        use_nearest_fallback=True,
    )
    OR3 = add_income_from_acs_s1901(
        OR2,
        tracts_path=str(ROOT / "BASE_FILES" / "OR_tract" / "tl_2025_41_tract.shp"),
        income_csv_path=str(ROOT / "BASE_FILES" / "OR-income.csv"),
        use_nearest_fallback=True,
    )

    # Step 3: Write final GeoJSONs
    AL3.to_file(str(ROOT / "AL_data" / "AL_precincts_full.geojson"), driver="GeoJSON")
    OR3.to_file(str(ROOT / "OR_data" / "OR_precincts_full.geojson"), driver="GeoJSON")
