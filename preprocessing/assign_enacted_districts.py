...
def main():
    ...
    assign_enacted_districts(
        precinct_path=str(ROOT / "BASE_FILES" / "AL-precincts-with-results.geojson"),
        districts_path=str(ROOT / "BASE_FILES" / "AL_Congressional_Districts_Shapefile" / "SP_Remedial_Plan_3 2023-10-05.shp"),
        out_path=str(ROOT / "AL_data" / "AL-precincts-with-results-enacted.geojson"),
    ...
    # ── Oregon ────────────────────────────────────────────────────────────
    assign_enacted_districts(
        precinct_path=str(ROOT / "BASE_FILES" / "OR-precincts-with-results.geojson"),
        districts_path=str(ROOT / "BASE_FILES" / "OR_Congressional_Districts.geojson"),
        out_path=str(ROOT / "OR_data" / "OR-precincts-with-results-enacted.geojson"),
        precinct_id_col="GEOID",
        district_id_col="DISTRICT",
    )

    # ── Post-process: cast enacted_cd to int for AL ───────────────────────
    al = gpd.read_file(str(ROOT / "AL_data" / "AL-precincts-with-results-enacted.geojson"))
    al["enacted_cd"] = al["enacted_cd"].astype(int)
    al.to_file(str(ROOT / "AL_data" / "AL-precincts-with-results-enacted.geojson"), driver="GeoJSON")

    # ── Post-process: cast enacted_cd to int for OR ───────────────────────
    or_ = gpd.read_file(str(ROOT / "OR_data" / "OR-precincts-with-results-enacted.geojson"))
    or_["enacted_cd"] = or_["enacted_cd"].astype(int)
    or_.to_file(str(ROOT / "OR_data" / "OR-precincts-with-results-enacted.geojson"), driver="GeoJSON")
