...
def main():
    ...
    build_precinct_adjacency_graph(
        precinct_geojson=str(ROOT / "AL_data" / "AL_precincts_full.geojson"),
        out_graph_json=str(ROOT / "AL_data" / "AL_graph.json"),
        out_graph_json2=str(ROOT / "seawulf_runs" / "AL" / "input" / "AL_graph.json"),
        min_shared_boundary_feet=200,
    )

    # Step 1: Build Oregon adjacency graph
    build_precinct_adjacency_graph(
        precinct_geojson=str(ROOT / "OR_data" / "OR_precincts_full.geojson"),
        out_graph_json=str(ROOT / "OR_data" / "OR_graph.json"),
        out_graph_json2=str(ROOT / "seawulf_runs" / "OR" / "input" / "OR_graph.json"),
        min_shared_boundary_feet=200,
    )
