...
def main():
    ...
    AL_graph = Graph.from_json(str(ROOT / "AL_data" / "AL_graph.json"))
    OR_graph = Graph.from_json(str(ROOT / "OR_data" / "OR_graph.json"))

    # ── Step 1: Validate attribute completeness for both graphs ───────────
    check_graph_attrs(AL_graph, "AL")
    check_graph_attrs(OR_graph, "OR")

    # ── Step 2: Build enacted partitions for QA ───────────────────────────
    updaters = {
    ...
    # ── Step 3: Print enacted partition QA ───────────────────────────────
    ...
    # ── Step 4: Cross-check node-level VAP sums against partition tallies ─
    ...
    # ── Step 5: Generate and save baselines for AL and OR ────────────────
    save_enacted_baseline(
        str(ROOT / "AL_data" / "AL_graph.json"),
        str(ROOT / "AL_data" / "AL_enacted_baseline.json"),
        str(ROOT / "seawulf_runs" / "AL" / "input" / "AL_enacted_baseline.json"),
        num_districts=7,
    )
    save_enacted_baseline(
        str(ROOT / "OR_data" / "OR_graph.json"),
        str(ROOT / "OR_data" / "OR_enacted_baseline.json"),
        str(ROOT / "seawulf_runs" / "OR" / "input" / "OR_enacted_baseline.json"),
        num_districts=6,
    )

    # ── Step 6: Save starting assignments ────────────────────────────────
    save_starting_assignment(
        str(ROOT / "AL_data" / "AL_graph.json"),
        str(ROOT / "AL_data" / "AL_starting_assignment_enacted.json"),
        str(ROOT / "seawulf_runs" / "AL" / "input" / "AL_starting_assignment_enacted.json"),
    )
    save_starting_assignment(
        str(ROOT / "OR_data" / "OR_graph.json"),
        str(ROOT / "OR_data" / "OR_starting_assignment_enacted.json"),
        str(ROOT / "seawulf_runs" / "OR" / "input" / "OR_starting_assignment_enacted.json"),
    )
