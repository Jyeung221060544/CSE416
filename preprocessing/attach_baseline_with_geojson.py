...
def main():
    ...
    attach_baseline_to_official_districts(
        districts_geojson_path=str(ROOT / "BASE_FILES" / "AL_Congressional_Districts_Shapefile" / "SP_Remedial_Plan_3 2023-10-05.shp"),
        baseline_json_path=str(ROOT / "AL_data" / "AL_enacted_baseline.json"),
        out_geojson_path=str(ROOT / "AL_data" / "AL_enacted_districts_with_stats.geojson"),
        out_geojson_path2=str(ROOT / "seawulf_runs" / "AL" / "input" / "AL_enacted_districts_with_stats.geojson"),
        district_col="DISTRICT",
    )

    # OR
    attach_baseline_to_official_districts(
        districts_geojson_path=str(ROOT / "BASE_FILES" / "OR_Congressional_Districts.geojson"),
        baseline_json_path=str(ROOT / "OR_data" / "OR_enacted_baseline.json"),
        out_geojson_path=str(ROOT / "OR_data" / "OR_enacted_districts_with_stats.geojson"),
        out_geojson_path2=str(ROOT / "seawulf_runs" / "OR" / "input" / "OR_enacted_districts_with_stats.geojson"),
        district_col="DISTRICT",
    )
